...
function makeTiff(dateOriginal = '2024:02:18 06:51:47', createDate = '2024:02:18 06:51:48') {
  ...
  // Exif IFD: DateTimeOriginal + CreateDate.
  u16le(bytes, 26, 2)
...
function makeHeic(tiff) {
  // Exif item payload starts with a 4-byte TIFF offset, then TIFF bytes.
  const exifPayload = new Uint8Array(4 + tiff.length)
  u32be(exifPayload, 0, 0)
  exifPayload.set(tiff, 4)

  // infe v2: fullbox + item_ID + protection index + item_type('Exif').
  ...
  // iloc v0, offset/length are 4-byte, no base offset.
  ...
  // extent offset patched after final meta size is known.
  u32be(ilocPayload, 18, exifPayload.length)
  const iloc = box('iloc', ilocPayload)

  const metaPayload = concat(new Uint8Array(4), iinf, iloc)
  ...
  // Locate iloc extent offset in the assembled meta box and patch it.
  // meta(8) + fullbox(4) + iinf + iloc(8) + iloc payload offset 14.
